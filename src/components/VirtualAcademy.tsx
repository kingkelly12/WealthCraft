...
export function TradingHub() {
  ...
  const handleBid = async (offerId: string) => {
    if (!user) {
      toast({
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          description: `Bid placed on ${offer.asset}`
        });
      ...
      toast({
        title: "Bid Placed!",
        description: `Your bid of $${parseFloat(amount).toLocaleString()} on ${offer.asset} has been placed`
      });
    } catch (error) {
  ...
  const handleNegotiate = (offerId: string) => {
    ...
    toast({
      ...
      description: `Negotiation request sent to ${offer?.seller}`,
    });
  };

  const handleBuyNow = async (offerId: string) => {
    if (!user) {
      toast({
        title: "Authentication Required",
    ...
    try {
      ...
      const { data: assetData, error: assetError } = await supabase
        ...
        .insert({
          user_id: user.id,
          name: offer.asset,
          asset_type: offer.category.toLowerCase().replace(' ', '_'),
          value: offer.askingPrice,
          purchase_price: offer.askingPrice
        });
      ...
      const { data: transactionData, error: transactionError } = await supabase
        ...
        .insert({
          ...
          description: `Purchased ${offer.asset}`
        });
      ...
      toast({
        ...
        description: `Successfully purchased ${offer.asset} for $${offer.askingPrice.toLocaleString()}`
      });
    } catch (error) {
